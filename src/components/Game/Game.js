...
const Game = () => {
  ...
  const persistState = async() => {
    // state variables in asyncstorage
    const dataForToday = {
      rows,
      curRow,
      curCol,
    ...
    try{
      const exsistingStateString = await AsyncStorage.getItem("@game");
      const exsistingState = exsistingStateString 
        ? JSON.parse(exsistingStateString) 
        : {};
      exsistingState[dayKey] = dataForToday;
      const dataString = JSON.stringify(exsistingState);
      await AsyncStorage.setItem("@game", dataString);
    }catch(e){
      console.log("Failed to write to asyncstorage", e);
    }
